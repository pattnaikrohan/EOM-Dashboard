import snowflake.connector
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.backends import default_backend

SF_ACCOUNT   = "SGLYREN-GG43054"
SF_USER      = "TEST_AI_AUTO"
SF_WAREHOUSE = "PROD_COMPUTE_WH"
SF_ROLE      = "PROD_ENGINEER"

PRIVATE_KEY_PEM = """-----BEGIN PRIVATE KEY-----

8QOHXsEQQry1/656UObS4Nvl
-----END PRIVATE KEY-----"""

def get_connection():
    p_key = serialization.load_pem_private_key(
        PRIVATE_KEY_PEM.encode(), password=None, backend=default_backend())
    pkb = p_key.private_bytes(
        encoding=serialization.Encoding.DER,
        format=serialization.PrivateFormat.PKCS8,
        encryption_algorithm=serialization.NoEncryption())
    return snowflake.connector.connect(
        account=SF_ACCOUNT, user=SF_USER, private_key=pkb,
        warehouse=SF_WAREHOUSE, role=SF_ROLE)

def execute_query(cur, query):
    print(f"Executing Query (First 100 chars): {query.strip()[:100].encode('ascii', 'ignore').decode('ascii')}...")
    try:
        cur.execute(query)
        if cur.description:
            rows = cur.fetchall()
            cols = [desc[0] for desc in cur.description]
            return True, rows, cols
        else:
            return True, None, None
    except Exception as e:
        print(f"FAILED: {str(e)}")
        return False, None, None

def main():
    print("Connecting to Snowflake...")
    conn = get_connection()
    cur = conn.cursor()
    
    # Read the SQL file
    sql_file = r'd:\EOM DASHBOARDS PROTO\database\VW_EOM_JOB_CHARGES.sql'
    with open(sql_file, 'r', encoding='utf-8') as f:
        sql_content = f.read()

    # Extract the CTE and SELECT from VW_EOM_JOB_CHARGES.sql
    query_parts = sql_content.split('CREATE OR REPLACE VIEW DEV.CORE.VW_EOM_JOB_CHARGES\nAS\n')
    if len(query_parts) > 1:
        # Get everything up to the second CREATE OR REPLACE VIEW
        select_query = query_parts[1].split('CREATE OR REPLACE VIEW')[0]
        # Append LIMIT 5
        select_query = select_query.strip().rstrip(';') + "\nLIMIT 5;"
        
        print("\n--- Testing VW_EOM_JOB_CHARGES query directly on PROD tables ---")
        success, rows, cols = execute_query(cur, select_query)
        if success and rows:
            print(f"{cols[0]:15} | {cols[1]:20} | {cols[2]:30} | {cols[3]:15} | {cols[4]:15}")
            print("-" * 105)
            for row in rows:
                print(f"{str(row[0])[:15]:15} | {str(row[1])[:20]:20} | {str(row[2])[:30]:30} | {str(row[3])[:15]:15} | {str(row[4])[:15]:15}")
        else:
            print("Failed to run VW_EOM_JOB_CHARGES query.")

    # Same for VW_EOM_JOBS_SUMMARY
    query_parts = sql_content.split('CREATE OR REPLACE VIEW DEV.CORE.VW_EOM_JOBS_SUMMARY\nAS\n')
    if len(query_parts) > 1:
        select_query = query_parts[1]
        select_query = select_query.strip().rstrip(';') + "\nLIMIT 5;"
        
        print("\n--- Testing VW_EOM_JOBS_SUMMARY query directly on PROD tables ---")
        success, rows, cols = execute_query(cur, select_query)
        if success and rows:
            print(f"{cols[0]:15} | {cols[1]:20} | {cols[2]:15} | {cols[3]:15} | {cols[4]:15} | {cols[5]:15}")
            print("-" * 105)
            for row in rows:
                print(f"{str(row[0])[:15]:15} | {str(row[1])[:20]:20} | {str(row[2])[:15]:15} | {str(row[3])[:15]:15} | {str(row[4])[:15]:15} | {str(row[5])[:15]:15}")
        else:
            print("Failed to run VW_EOM_JOBS_SUMMARY query.")

    cur.close()
    conn.close()
    print("\nTests completed successfully.")

if __name__ == "__main__":
    main()
